...
$col_vod=array('d_id', 'd_name', 'd_subname', 'd_enname', 'd_letter', 'd_color', 'd_pic', 'd_picthumb', 'd_picslide', 'd_starring', 'd_directed', 'd_tag', 'd_remarks', 'd_area', 'd_lang', 'd_year', 'd_type', 'd_type_expand', 'd_class', 'd_hide', 'd_lock', 'd_state','d_psd', 'd_level', 'd_usergroup', 'd_stint', 'd_stintdown', 'd_hits', 'd_dayhits', 'd_weekhits', 'd_monthhits', 'd_duration', 'd_up', 'd_down', 'd_score','d_scoreall', 'd_scorenum', 'd_addtime', 'd_time', 'd_hitstime', 'd_maketime', 'd_content', 'd_playfrom', 'd_playserver', 'd_playnote', 'd_playurl', 'd_downfrom', 'd_downserver', 'd_downnote', 'd_downurl');
...
elseif($method=='batch')
{
	...
	$psd=$p['psd']; if(isN($psd)){ $psd=999; } else { $psd=intval($psd); }
	...
	if($psd!=999){
		if($psd==0){ $where.=' and d_psd=0 '; } else{ $where.=' and d_psd>0 '; }
	}
	if(!empty($play)){
		if($play=='no'){
	...
	elseif($ckdel=='2' || $ckdel=='3'){
		...
		if($nums==0){
			...
			$url = '?m=vod-batch-type-'.$type.'-topic-'.$topic.'-level-'.$level.'-hide-'.$hide.'-lock-'.$lock.'-by-'.$by.'-pg-'.($page+1).'-wd-'.urlencode($wd).'-repeat-'.$repeat.'-repeatlen-'.$repeatlen.'-state-'.$state.'-psd-'.$psd.'-pic-'.$pic.'-play-'.$play.'-down-'.$down.'-server-'.$server.'-area-'.urlencode($area).'-lang-'.urlencode($lang).'-ckdel-'.$ckdel;
	...
	elseif($ckrq=='1'|| $cktj=='1' || $cklock=='1' || $ckhide=='1'){
		...
		if($page>$pagecount){
			...
			$url = '?m=vod-batch-type-'.$type.'-topic-'.$topic.'-level-'.$level.'-hide-'.$hide.'-lock-'.$lock.'-by-'.$by.'-pg-'.($page+1).'-wd-'.urlencode($wd).'-repeat-'.$repeat.'-repeatlen-'.$repeatlen.'-state-'.$state.'-psd-'.$psd.'-pic-'.$pic.'-play-'.$play.'-down-'.$down.'-server-'.$server.'-area-'.urlencode($area).'-lang-'.urlencode($lang).'-ckrq-'.$ckrq.'-batch_hits1-'.$batch_hits1.'-batch_hits2-'.$batch_hits2.'-cktj-'.$cktj.'-batch_level-'.$batch_level.'-cklock-'.$cklock.'-batch_lock-'.$batch_lock.'-ckhide-'.$ckhide.'-batch_ckhide-'.$batch_ckhide;
...
elseif($method=='list')
{
	...
	$arr=array(
		...
		array('a'=>'psd','c'=>$psd,'n'=>array('无密码','带密码'),'v'=>array(0,1)),
	...
	if($repeat_status==1){
		...
		$sql='select a.`d_id`, `d_name`, `d_enname`, `d_color`, `d_pic`, `d_remarks`, `d_type`, `d_type_expand` ,`d_hide`, `d_lock`, `d_psd`, `d_level`,  `d_hits`,  `d_addtime`, `d_time`, `d_maketime`, `d_playfrom`, `d_downfrom`,b.d_name1 from {pre}vod a INNER JOIN (SELECT d_id,left(d_name,'.$repeatlen.') as d_name1 from {pre}vod where CHAR_LENGTH(d_name)>='.$repeatlen.' ) b on a.d_id = b.d_id  INNER JOIN (select d_name1 from tmptable) c on b.d_name1 = c.d_name1 where 1=1 ORDER BY d_name asc  limit '.($MAC['app']['pagesize'] * ($page-1)) .",".$MAC['app']['pagesize'];
		$rs = $db->query($sql);
	}
	else{
		$sql = 'SELECT count(*) FROM {pre}vod'.$tmptab.' where 1=1 '.$where;
		$nums = $db->getOne($sql);
		$sql = "SELECT `d_id`, `d_name`, `d_enname`, `d_color`, `d_pic`, `d_remarks`, `d_type`, `d_type_expand` ,`d_hide`, `d_lock`, `d_psd`, `d_level`,  `d_hits`,  `d_addtime`, `d_time`,`d_maketime`, `d_playfrom`, `d_downfrom`  FROM {pre}vod".$tmptab." where 1=1 ";
		$sql .= $where;
	...
	while ($row = $db ->fetch_array($rs))
	{
		...
		$valarr['d_psd'] = $row['d_psd']==0 ? '' : '<font color=red>[密码:'.$row['d_psd'].']</font>';
	...
	$pageurl = '?m=vod-list-type-'.$type.'-topic-'.$topic.'-level-'.$level.'-hide-'.$hide.'-lock-'.$lock.'-by-'.$by.'-pg-{pg}-wd-'.urlencode($wd).'-repeat-'.$repeat.'-repeatlen-'.$repeatlen.'-psd-'.$psd.'-pic-'.$pic.'-play-'.$play.'-down-'.$down.'-server-'.$server.'-area-'.urlencode($area).'-lang-'.urlencode($lang);
	$pages = '共'.$nums.'条数据&nbsp;当前:'.$page.'/'.$pagecount.'页&nbsp;'.pageshow($page,$pagecount,6,$pageurl,'pagego(\''.$pageurl.'\','.$pagecount.')');
	$plt->set_var('pages', $pages );
}
