...
if($method=='configsave')
{
	...
	$config['app']['quanju'] = trim(be('post','app_quanju'));
	$config['app']['wxpass'] = trim(be('post','app_wxpass'));	
...
elseif($method=='configplaysave')
{
	...
	$mac_ydwidth = be('post','mac_ydwidth');
	$mac_ydheight = be('post','mac_ydheight');
	...
	$fc = regReplace($fc,"var\smac_flag\=*\"*(\S+?)'*\"*\;","var mac_flag=\"".$mac_flag."\";");
	$fc = regReplace($fc,"var\smac_second\=*\"*(\S+?)'*\"*\;","var mac_second=\"".$mac_second."\";");
	$fc = regReplace($fc,"var\smac_width\=*\"*(\S+?)'*\"*\;","var mac_width=\"".$mac_width."\";");
	$fc = regReplace($fc,"var\smac_height\=*\"*(\S+?)'*\"*\;","var mac_height=\"".$mac_height."\";");
	$fc = regReplace($fc,"var\smac_ydwidth\=*\"*(\S+?)'*\"*\;","var mac_ydwidth=\"".$mac_ydwidth."\";");
	$fc = regReplace($fc,"var\smac_ydheight\=*\"*(\S+?)'*\"*\;","var mac_ydheight=\"".$mac_ydheight."\";");
	$fc = regReplace($fc,"var\smac_widthpop\=*\"*(\S+?)'*\"*\;","var mac_widthpop=\"".$mac_widthpop."\";");
	$fc = regReplace($fc,"var\smac_heightpop\=*\"*(\S+?)'*\"*\;","var mac_heightpop=\"".$mac_heightpop."\";");
	$fc = regReplace($fc,"var\smac_showtop\=*\"*(\S+?)'*\"*\;","var mac_showtop=\"".$mac_showtop."\";");
	$fc = regReplace($fc,"var\smac_showlist\=*\"*(\S+?)'*\"*\;","var mac_showlist=\"".$mac_showlist."\";");
	$fc = regReplace($fc,"var\smac_autofull\=*\"*(\S+?)'*\"*\;","var mac_autofull=\"".$mac_autofull."\";");
...
elseif($method=='configplay')
{
	...
	if(!file_exists($fp)){ $fp .= '.bak'; }
	$fc = file_get_contents( $fp );
	$mac_flag = regMatch($fc,"var\smac_flag\=*\"*(\S+?)'*\"*\;");
	$mac_second = regMatch($fc,"var\smac_second\=*\"*(\S+?)'*\"*\;");
	$mac_width = regMatch($fc,"var\smac_width\=*\"*(\S+?)'*\"*\;");
	$mac_height = regMatch($fc,"var\smac_height\=*\"*(\S+?)'*\"*\;");
	$mac_ydwidth = regMatch($fc,"var\smac_ydwidth\=*\"*(\S+?)'*\"*\;");
	$mac_ydheight = regMatch($fc,"var\smac_ydheight\=*\"*(\S+?)'*\"*\;");
	$mac_widthpop= regMatch($fc,"var\smac_widthpop\=*\"*(\S+?)'*\"*\;");
	$mac_heightpop=regMatch($fc,"var\smac_heightpop\=*\"*(\S+?)'*\"*\;");
	$mac_showtop = regMatch($fc,"var\smac_showtop\=*\"*(\S+?)'*\"*\;");
	$mac_showlist = regMatch($fc,"var\smac_showlist\=*\"*(\S+?)'*\"*\;");
	$mac_autofull=regMatch($fc,"var\smac_autofull\=*\"*(\S+?)'*\"*\;");
	...
	$colarr=array('mac_second','mac_width','mac_height','mac_ydwidth','mac_ydheight','mac_widthpop','mac_heightpop','mac_buffer','mac_prestrain','mac_colors');
	$valarr=array($mac_second,$mac_width,$mac_height,$mac_ydwidth,$mac_ydheight,$mac_widthpop,$mac_heightpop,$mac_buffer,$mac_prestrain,$mac_colors);
	for($i=0;$i<count($colarr);$i++){
		$n = $colarr[$i];
		$v = $valarr[$i];
		$plt->set_var($n, $v );
...
{
	$vod_xmurl = be("post", "vod_xmurl");    
    fwrite(fopen("../inc/ym.xml","wb"),$vod_xmurl);
	$config = $MAC;

	$config['api']['vod']['status'] = intval(trim(be('post','vod_status')));
	$config['api']['vod']['ff'] = intval(trim(be('post','vod_ff')));
...
elseif($method=='configapi')
{
	...
	$vod_xmurl = file_get_contents("../inc/ym.xml"); 
	$plt->set_var('vod_xmurl', $vod_xmurl );
...
elseif($method=='configweixinsave'){
	...
	$config['weixin']['passwordkey'] = trim(be('post','weixin_passwordkey'));	
...
elseif($method=='configweixin'){
	...
	$arr=array(
		...
		array('p'=>'weixin','a'=>'chufa','c'=>$MAC['weixin']['chufa'],'t'=>0,'n'=>array('关闭','开启') ,'v'=>array(0,1) ),
		array('p'=>'weixin','a'=>'zhezhao','c'=>$MAC['weixin']['zhezhao'],'t'=>0,'n'=>array('关闭','开启') ,'v'=>array(0,1) )
	);
	foreach($arr as $a){
